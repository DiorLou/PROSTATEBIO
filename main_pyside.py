...
# --- TCP 消息接收线程 ---
# 这是一个自定义的 QThread 类，用于在后台线程中接收TCP数据，
# 以免阻塞主GUI线程，保持界面的响应性。这是GUI编程中的一个重要模式。
...
class RobotControlWindow(QMainWindow):
    ...
    def connect_tcp(self):
        ...
        try:
            ...
            self.client_socket.settimeout(None)  # 连接成功后取消超时，以便长时间接收数据。
...
if __name__ == "__main__":
    # 创建 QApplication 实例，这是所有Qt应用的必需。
    app = QApplication(sys.argv)
    # 创建主窗口。
    window = RobotControlWindow()
    # 显示主窗口。
    window.show()
    # 启动Qt事件循环。这是一个阻塞调用，直到应用退出。
    sys.exit(app.exec_())